...
# Load in first file
dat1 <- readNifti("Mean BOLD Signal Re-creation/MaxMun_c_0051342_func_preproc.nii.gz")
dim(dat1)
...
ABIDE <- read.table("Preprocessed Data/dparsf_nofilt_noglobal_rois_aal/MaxMun_c_0051342_rois_aal.1D",
                    comment.char = "", header = TRUE)

# Plot overlapping BOLD signals - they are similar, but not exact
index=4
plot(ABIDE[,index],type = 'l',ylim=c(-4,3))
