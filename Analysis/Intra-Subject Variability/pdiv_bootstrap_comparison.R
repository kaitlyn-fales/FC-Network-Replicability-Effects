...
dat_result <- read.csv("Analysis/Intra-Subject Variability/pdiv_result.csv")
...
for (b in 1:100){
  ...
  boot_result <- read.csv(paste0("Analysis/Intra-Subject Variability/Bootstrap Results/pdiv_bootstrap",b,".csv"))
...
save(baseline_curves, file = "Analysis/Intra-Subject Variability/Baseline Curves/baseline_curves_pdiv_pipeline.RData")

# RESUME from here if you do not run the bootstrap 
load("Analysis/Intra-Subject Variability/Baseline Curves/baseline_curves_pdiv_pipeline.RData")
...
for (i in 1:length(comparisons)){
  ...
  plot(densities[[1]], 
       ...
       ylim=c(0,6), main = paste0("Bootstrap Dens: ",comparisons[i]))
  for (j in 1:100){
    lines(densities[[j]])
  }
  lines(x, boot_avg, col = "red", lwd = 2)
...
# Save baseline curves
save(baseline_curves, file = "Analysis/Intra-Subject Variability/Baseline Curves/baseline_curves_pdiv_atlas.RData")

# RESUME from here if you do not run the bootstrap 
load("Analysis/Intra-Subject Variability/Baseline Curves/baseline_curves_pdiv_atlas.RData")
...
for (i in 1:length(comparisons)){
  plot <- dat_summary %>% filter(Pair == comparisons[i]) %>% 
    ...
    geom_density(aes(x = mean_dat, color="Observed Difference"), 
                 fill = "pink", alpha = 0.5, linewidth = 1) + 
    labs(title = paste0(comparisons[i]),
         x = "Mean Ind. Portrait Divergence",
         y = "Density") + xlim(0,1) + ylim(0,6) +
    scale_color_manual(name = "Source", values = c("blue", "red")) +
    theme_classic() +
    guides(color = guide_legend(override.aes = list(linetype = c(0,1), shape = 22, fill = c("lightblue", "pink"), 
                                                    size = 5))) +
